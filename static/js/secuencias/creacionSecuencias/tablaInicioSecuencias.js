...
function aplicarVisibilidadColumnas() {
  const toggleInvalidar = document.getElementById('toggleInvalidar');
  const toggleRevisar = document.getElementById('toggleRevisar');
...
<td class="btn-detalles icon-hover-zoom text-secondary"
    data-id="${item.id}"
    data-endpoint="/crear_secuencias_en_curso/listado_secuencias_registradas"
    title="Ver Detalles"
  ...
  <span class="fa fa-camera-retro">${item.nombre}</span>
      ...
      <td>${item.sistema || ''}</td>
      <td class="col-invalida hidden-col">
        <i class="fa fa-thumbs-down me-1"></i>
        <input type="checkbox" class="select-item" data-id="${item.id}" />
      </td>
      <td class="col-revisar d-none">
        <i class="fa fa-thumbs-up me-1"></i>
        <input type="checkbox" class="checkbox-revisar" data-id="${item.id}">
      </td>
      <td class="text-center col-opciones">
        ${
          item.protocolo === "None" && item.metodo === "None"
            ? `<button onclick="editarSecuenciaProceso(${item.id})" title="Editar"><i class="fa fa-edit text-primary text-lg"></i></button>`
            : item.protocolo === "None" && item.protocolo_proceso === "None"
            ? `<button onclick="editarSecuenciaOtro(${item.id})" title="Editar"><i class="fa fa-edit text-primary text-lg"></i></button>`
            : item.protocolo !== "None"
            ? `<button onclick="editarSecuenciaMetodo(${item.id})" title="Editar"><i class="fa fa-edit text-primary text-lg"></i></button>`
            : ''
        }
        &nbsp;
        ${
          item.protocolo !== "None"
            ? `<button onclick="duplicarSecuenciaMetodo(${item.id})" title="Agregar"><i class="fa fa-plus-circle text-success text-lg"></i></button>`
            : `<button onclick="duplicarSecuenciaMuestras(${item.id})" title="Agregar"><i class="fa fa-plus-circle text-success text-lg"></i></button>`
        }
      </td>
    </tr>
  `).join('');
...
async function getDataAndRender() {
  try {
    const response = await fetch("/crear_secuencias_en_curso/listado_secuencias_registradas");
